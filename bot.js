...
function respond() {
  var request = JSON.parse(this.req.chunks[0]),botHugs = /^\/hugs/;
      ...
      botRegexSiege = /^\/siege/; botDoc = /^\/doc/; botSf = /^\/sf/; botWake = /^\/wakeup/; botSd = /^\/sd/;  botDet = /^\/det/;  botBaseball = /^\/baseball/; botSteve = /^\/steve/;
  ...
  if(request.text && botRegex.test(request.text)) {
    ...
    else if (request.text && botWake.test(request.text)) {
    this.res.writeHead(200);
    postMessage("Yes, Sir Nish");
    this.res.end();
  }
